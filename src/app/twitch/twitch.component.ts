...
export class TwitchComponent implements OnInit {
	...
	ngOnInit() {
		this.http
			.get(
				'https://api.twitch.tv/kraken/users?login=vazthink,yowonky,cohhcarnage,kinggothalion,p4wnyhof',
				httpOptions
			)
			.subscribe(data => {
	...
	getOnlineState(id, i) {
		this.http
			.get('https://api.twitch.tv/kraken/streams/' + id, httpOptions)
			.subscribe(data => {
	...
	getHighlightedStreamer() {
		var streamer = this.twitchUsers[
			Math.floor(Math.random() * this.twitchUsers.length)
		];
	...
	addTwitchUser(data) {
		for (let i = 0; i < data._total; i++) {
			...
			twitchUser.stream_url =
				'https://www.twitch.tv/' + data.users[i].display_name;
